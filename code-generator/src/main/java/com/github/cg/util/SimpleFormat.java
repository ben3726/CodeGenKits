...
public class SimpleFormat {
    ...
    public static String humpToUnderline(String para) {
        ...
        return var1.toString().toLowerCase().startsWith("_") ? var1.toString().toLowerCase().substring(1)
    ...
    public static String humpToShortbar(String para) {
        ...
        return var1.toString().toLowerCase().startsWith("-") ? var1.toString().toLowerCase().substring(1)
                : var1.toString().toLowerCase();
    }

    public static void main(String[] args) {
        log.info(humpToShortbar("cgDemo"));
    }

    public String number(Object obj) {
        obj = obj != null && obj.toString().length() != 0 ? obj : 0;
        return obj.toString().equalsIgnoreCase("NaN") ? "NaN"
                : (new DecimalFormat("0.00")).format(Double.parseDouble(obj.toString()));
    }

    public String number(Object obj, String pattern) {
        obj = obj != null && obj.toString().length() != 0 ? obj : 0;
        return obj.toString().equalsIgnoreCase("NaN") ? "NaN"
                : (new DecimalFormat(pattern)).format(Double.parseDouble(obj.toString()));
    }

    public String round(Object obj) {
        obj = obj != null && obj.toString().length() != 0 ? obj : 0;
        return obj.toString().equalsIgnoreCase("NaN") ? "NaN"
                : (new DecimalFormat("0")).format(Double.parseDouble(obj.toString()));
    }
